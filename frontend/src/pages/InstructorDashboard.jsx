...
const InstructorDashboard = () => {
    ...
    const [searchQuery, setSearchQuery] = useState(() => {
        const saved = localStorage.getItem("InstructorDashboard_searchQuery");
        return saved || "";
    });
    const [roleFilter, setRoleFilter] = useState(() => {
        const saved = localStorage.getItem("InstructorDashboard_roleFilter");
        return saved || "all";
    });
    const [sortBy, setSortBy] = useState(() => {
        const saved = localStorage.getItem("InstructorDashboard_sortBy");
        return saved || "name";
    });
    const [sortOrder, setSortOrder] = useState(() => {
        const saved = localStorage.getItem("InstructorDashboard_sortOrder");
        return saved || "asc";
    });

    // Pagination state
    const [currentPage, setCurrentPage] = useState(() => {
        const saved = parseInt(localStorage.getItem("InstructorDashboard_currentPage"), 10);
    ...
    const debouncedSetSearch = useRef(
        debounce((value) => {
            ...
            localStorage.setItem("InstructorDashboard_searchQuery", value);
        }, 300)
    ).current;
    ...
    useEffect(() => {
        localStorage.setItem("InstructorDashboard_roleFilter", roleFilter);
    }, [roleFilter]);

    useEffect(() => {
        localStorage.setItem("InstructorDashboard_sortBy", sortBy);
    }, [sortBy]);

    useEffect(() => {
        localStorage.setItem("InstructorDashboard_sortOrder", sortOrder);
    }, [sortOrder]);
    ...
    useEffect(() => {
        if (currentPage !== 1) {
            setCurrentPage(1);
            localStorage.setItem("InstructorDashboard_currentPage", "1");
        }
    ...
    useEffect(() => {
        if (currentPage > totalPages && totalPages > 0) {
            ...
            localStorage.setItem("InstructorDashboard_currentPage", totalPages.toString());
        }
    }, [currentPage, totalPages]);

    // Persist currentPage to localStorage
    useEffect(() => {
        localStorage.setItem("InstructorDashboard_currentPage", currentPage.toString());
    }, [currentPage]);
